...
| random_images_dir   | Если установлен, выбирает для пользователя аватарку из директории по следующей формуле: `user_id` % `N`, где `N` - количество картинок в директории.  | "path/imgs                                               | None         |
